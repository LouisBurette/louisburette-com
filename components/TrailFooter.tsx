...
function buildPolylines(count: number, stroke: string, baseOpacity: number, opacityStep: number, offsetY: number) {
  return Array.from({ length: count }, (_, i) => {
    const dy = i * offsetY;
    const pts = ELEVATION_POINTS.split(' ').map(p => {
      const [x, y] = p.split(',').map(Number);
      return `${x},${(y + dy).toFixed(1)}`;
    }).join(' ');
    return <polyline key={i} points={pts} fill="none" stroke={stroke} strokeWidth="1.3" opacity={(baseOpacity - i * opacityStep).toFixed(3)} />;
  });
}
    ...
    <section id="trail-section" ref={sectionRef} style={{ padding: '80px 48px 44px', background: '#1B3A22', color: '#F5EFE5', position: 'relative', overflow: 'hidden', borderTop: '3px solid #1A1714', minHeight: '440px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'relative', zIndex: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '20px', marginTop: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '10px' }}>
          <a id="trail-cta" ref={ctaRef} href="mailto:hello@louisburette.com"
